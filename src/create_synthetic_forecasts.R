...
site<-'NHGC1'  #site label
site_num<-which(col_names==site)    #site num for HEFS matrix
...
var_lag<-1
var_mean<-F
var_mod<-'svar' # 'bvar' for BigVAR lasso penalized, 'svar' for Bigtime sparseVAR model, 'varx' for rmgarch robust least squares model
use_sigest<-T
distn<-'sep'
parallel<-T
use_mpi<-T
...
#rearrange fwd looking forecast
hefs<-fwd_forecast_rearrange(hefs_forward[site_num,,,])

#synch obs and hefs to fit period
obs_fit <- obs[which(ixx_obs==fit_start):which(ixx_obs==fit_end),site]
hefs_fit <- hefs[,which(ixx_hefs==fit_start):which(ixx_hefs==fit_end),]

#-----------------------------------------
#1) Fit conditional expectation model
...
#-----------------------------------------
#2) Fit linear heteroscedastic model
...
#-----------------------------------------
#3) Fit VAR model to normalized errors
...
#---------------------------------------------
#4) Fit SEP distribution to VAR residuals by ensemble, season, and K
...
#5) Generate synthetic forecasts
print(paste('start syngen',Sys.time()))

cumul_samp_span<-15
ixx_sim <- as.POSIXlt(seq(as.Date(gen_start),as.Date(gen_end),by='day'),tz = "UTC")
#update VAR or AR model indicator for synthetic generation
var_ar<-'var'
if(var_mod=='ar'){var_ar<-'ar'}
#5a. Function inputs
#syn_gen<-function(n_samp,obs,obs_idx,at_mat,sep_par,cexp_fit,norm_fit,var_coefs,lag,cumul_samp_span,use_mean,seasonal,fit_start,fit_end,gen_start,gen_end,parallel)

syn_hefs_fwd <- array(NA,c(n_samp,n_sites,n_ens,length(ixx_sim),leads))

syn_gen_out <- syn_gen(n_samp,obs=obs[,site_num],obs_idx=ixx_obs,at_mat,sep_par=sep_param,distn,cexp_fit=loess_fit,norm_fit,var_coefs,var_ar=var_ar,lag=var_lag,cumul_samp_span,use_mean=var_mean,seasonal,fit_start,fit_end,gen_start,gen_end,use_mpi)

syn_hefs_fwd[,site_num,,,] <- syn_gen_out[[1]]
syn_err <- syn_gen_out[[2]]

saveRDS(syn_hefs_fwd,file=paste('./out/syn_hefs_forward_',vers,'.rds',sep=''))
saveRDS(syn_err,file=paste('./out/syn_err_',vers,'.rds',sep=''))
